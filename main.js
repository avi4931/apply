$(document).ready(function () {
  $(".recruiter-carousel").owlCarousel({
    loop: true,
    margin: 10,
    nav: false,
    dots: false,
  ...
  $(".about-image-container").owlCarousel({
    loop: true,
    margin: 10,
    nav: false,
    dots: false,
...
var c_a = [
  "",
  "PGCC - 1 year Weekend Mode | PGDM 2-year Weekend Mode | PGDM Industry Integrated ( For Working Professionals) | PG Diploma in Business Intelligence & Analytics",
  "PGDM with International Immersion Program | PGDM (General) | PGDM in Product Management | PGDM in Data Science and Business Analytics | PGDM in Fintech and New Age Financial Services | PGDM with One Semester Abroad | Global PGDM | PGDM in Health Care Management | MBA | PGDM | PGDM in Fintech",
  "B.com Evening | B.Com. | B.Com. with ACCA | BBA | BBA in Aviation Management | BBA with CMA | BCA | M.Com.",
  "BBA LLB | Certificate Course in Human Rights | Certification Course in Constitutional Law | LLB | PG Certificate in Cyber Laws and IT | PG Certificate in Intellectual Property Rights",
  "Fellow Program in Management | Ph.D. in Commerce | Ph.D. in Management"
];

function print_institute(select_id) {
  var select = document.getElementById(select_id);
  select.length = 1; // Keep only "Select Institute"
  for (var i = 0; i < institute_arr.length; i++) {
    select.options[select.length] = new Option(institute_arr[i], institute_arr[i]);
  }
}

function print_course(select_id, index) {
  var select = document.getElementById(select_id);
  select.length = 1; // Keep only "Select Course"
  if (index < 0 || index >= c_a.length) return;
  var courses = c_a[index].split("|");
  for (var i = 0; i < courses.length; i++) {
    var course = courses[i].trim();
    if (course) {
      select.options[select.length] = new Option(course, course);
    }
  }
}

// Call this on page load to initialize institutes

print_institute("institute");
